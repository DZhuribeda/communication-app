...
import { DtlsParameters, MediaKind, Producer, RtpCapabilities, RtpParameters, SctpCapabilities, WebRtcTransport, Consumer, DataProducer, DataConsumer } from 'mediasoup/node/lib/types';
...
export class MediaService {
    ...
    _getJoinedPeers(room: MediaRoom, excludedUserId: string) {
        this.logger.log(`_getJoinedPeers ${room.peers.length} ${room.peers.filter((peer) => peer.id !== excludedUserId).map(p => p.id)}`);
        ...
        for (const room of Object.values(this.rooms)) {
            ...
            if (peer) {
                ...
                if (room.peers.length === 0) {
                    this.logger.log(`last Peer in the room left, closing the room [roomId:${room.id}]`);
    ...
    async createRoom(socket: any, userId: string, roomId: string): Promise<MediaRoom> {
        const router = await this.worker.createRouter({ ...mediasoupConfig.routerOptions });
        const room = {
            id: roomId,
            router,
            peers: [{
    ...
    async createWebRtcTransport(
        ...
        transport.on('sctpstatechange', (sctpState) => {
            this.logger.log(`WebRtc Transport "sctpstatechange" event [sctpState:${sctpState}]`);
        });
    ...
    async produce(
        ...
        producer.on('score', (score) => {
            this.logger.log(`producer "score" event [producerId:${producer.id}: ${score}]`);
            const socket = room.peers.find(peer => peer.id === userId).socket;
        ...
        producer.on('videoorientationchange', (videoOrientation) => {
            this.logger.log(`producer "videoorientationchange" event [producerId:${producer.id}, videoOrientation:${videoOrientation}]`);
        });
        ...
        producer.on('trace', (trace) => {
            this.logger.log(`producer "trace" event [producerId:${producer.id}, trace.type:${trace.type}, trace:${trace}]`);
        });
    ...
    async _createConsumer({ room, consumerPeer, producerPeer, producer }: {
        room: MediaRoom,
        ...
        const transport = Object.values(consumerPeer.transports).find((t) => t.appData.consuming);
        ...
        consumer.on('producerpause', () => {
            this.logger.log(`consumer "producerpause" event [consumerId:${consumer.id}]`);
            consumerPeer.socket.emit('consumerPaused', { consumerId: consumer.id })
            // .catch(() => { });
        });

        consumer.on('producerresume', () => {
            this.logger.log('consumer "producerresume" event [consumerId:%s]', consumer.id);
        ...
        consumer.on('score', (score) => {
            ...
            consumerPeer.socket.emit('consumerScore', { consumerId: consumer.id, score })
            // .catch(() => { });
        });
        ...
        consumer.on('trace', (trace) => {
            this.logger.debug(`consumer "trace" event [producerId:${consumer.id}, trace.type:${trace.type}, trace:{trace}]`);
        });
    ...
    async _createDataConsumer({
        ...
        // Must take the Transport the remote Peer is using for consuming.
        const transport = Object.values(dataConsumerPeer.transports)
        ...
        dataConsumer.on('dataproducerclose', () => {
            ...
            dataConsumerPeer.socket.emit('dataConsumerClosed', { dataConsumerId: dataConsumer.id })
        });
