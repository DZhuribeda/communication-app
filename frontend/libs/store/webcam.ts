...
export const createWebcamSlice = (set: SetState<AppState>, get: GetState<AppState>) => ({
    webcamProducer: null,
    webcams: new Map(),
    webcam: null,
    ...
    enableWebcam: async () => {
        ...
        try {
            ...
            if (forceH264) {
                codec = get().device?.rtpCapabilities?.codecs
            ...
            if (useSimulcast) {
                ...
                const firstVideoCodec = get().device
                    ?.rtpCapabilities
                    ?.codecs
                    ?.find((c) => c.kind === 'video');
            ...
            console.debug('enableWebcam() | calling store.dispatch(stateActions.setWebcamProducer(webcamProducer))');
    ...
    disableWebcam: async () => {
        ...
        try {
            ...
            await asyncEmit(socket, 'closeProducer', { roomId: roomId, producerId: webcamProducer.id })
        }
        catch (error) {
            // TODO: handle error
            // store.dispatch(requestActions.notify(
